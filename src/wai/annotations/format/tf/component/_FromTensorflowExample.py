...
from ..utils import (
    extract_feature,
    image_info_from_example,
    polygon_from_mask,
    TensorflowExampleExternalFormat
)


class FromTensorflowExample(
    RequiresNoFinalisation,
    ProcessorComponent[TensorflowExampleExternalFormat, ImageObjectDetectionInstance]
):
    """
    ...
    def process_element(
            self,
            element: TensorflowExampleExternalFormat,
        ...
        image_info: Image = image_info_from_example(element)

        # Extract the located object data from the instance
        lefts = extract_feature(element.features, 'image/object/bbox/xmin')
        rights = extract_feature(element.features, 'image/object/bbox/xmax')
        tops = extract_feature(element.features, 'image/object/bbox/ymin')
        bottoms = extract_feature(element.features, 'image/object/bbox/ymax')
        labels = list(map(decode_utf_8, extract_feature(element.features, 'image/object/class/text')))

        # Extract the optional mask information
        masks = extract_feature(element.features, 'image/object/mask')
        if len(masks) == 0:
            masks = [b""] * len(lefts)
